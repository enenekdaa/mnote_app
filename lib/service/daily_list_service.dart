...
String resultURL;
Future<List<Daily_model>> getDailyList(String dataURL) async {
  resultURL = dataURL;
  ...
  final response = await http.post(dataURL ,
    ...
    body: {'page_no' : '1' , 'search' : '하루' },
  );

  if (response.statusCode == 200) {
    final jsonResponse = json.decode(response.body);

    final items = (jsonResponse['daily_list'] as List).map((i) => new Daily_model.fromJson(i));
