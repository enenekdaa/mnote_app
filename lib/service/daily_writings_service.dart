...
String resultURL;
Future<List<Daily_writings>> getDailyWritingsList(String dataURL) async {
  resultURL = dataURL;
  ...
  final response = await http.post(dataURL ,
    ...
    body: {'page_no' : '1' , 'daily_list_no' : '4' },
  );

  if (response.statusCode == 200) {
    final jsonResponse = json.decode(response.body);

    final items = (jsonResponse['daily_writings'] as List).map((i) => new Daily_writings.fromJson(i));
